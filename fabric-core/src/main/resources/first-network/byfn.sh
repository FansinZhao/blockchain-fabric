...
function networkUp() {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_MYSQL  up -d 2>&1
  else
...
#    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config/ ./org3-artifacts/crypto-config/ channel-artifacts/org3.json
    rm -rf channel-artifacts/ channel-artifacts/ crypto-config/ ./org3-artifacts/crypto-config/ channel-artifacts/org3.json
