...
DROP TABLE IF EXISTS `fabric_cache`;
CREATE TABLE `fabric_cache` (
  `id` BIGINT(19) NOT NULL AUTO_INCREMENT,
  `username` VARCHAR(20) NOT NULL COMMENT '用户名',
  `is_admin` INT (1) DEFAULT 0 COMMENT '是否admin',
  `data` blob DEFAULT NULL COMMENT '数据',
  `create_datetime` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建日期时间',
  `update_datetime` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新日期时间',
  PRIMARY KEY (`id`),
  KEY `INDEX_UPDATE_DATETIME` (`update_datetime` ),
  KEY `INDEX_CREATE_DATETIME` (`create_datetime`)
) ENGINE=INNODB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8 COMMENT='fabric缓存表';